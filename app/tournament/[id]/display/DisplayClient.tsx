...
export default function DisplayClient({ tournament, sortedPlayers, roundDetails, totalRounds }: Props) {
  ...
  const medals = ['🥇', '🥈', '🥉'];
  const isFinalRound = roundDetails?.round_number === totalRounds;
              ...
              <div
                ...
                className={`w-4 h-4 rounded-full transition-all ${
                  i + 1 < tournament.current_round_number
                    ? 'bg-blue-500'
                    : i + 1 === tournament.current_round_number
                    ? 'bg-white ring-2 ring-blue-400 scale-125'
                    : 'bg-blue-900'
                }`}
              />
            ))}
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {sortedPlayers.map((player, index) => (
                  <tr
                    key={player.id}
                    className={`border-b border-blue-900/40 transition-colors ${
                      index === 0
                        ? 'bg-yellow-500/10'
                        : index === 1
                        ? 'bg-slate-400/5'
                  ...
                  >
                    ...
                    <td className="px-4 py-2.5 font-semibold text-white text-right">
                      {player.name}
                    </td>
                    <td className="px-3 py-2.5 text-center font-bold text-blue-300 text-base">
                      {player.total_points}
                    </td>
                    <td className={`px-3 py-2.5 text-center font-medium text-sm ${
                      player.total_diff > 0 ? 'text-green-400' : player.total_diff < 0 ? 'text-red-400' : 'text-blue-600'
                    }`}>
            ...
            <div className="bg-[#0f2347] border border-blue-900 rounded-2xl px-5 py-4">
              ...
              <div className="flex flex-wrap gap-2">
                {roundDetails.resting_players.map((p) => (
                  <span
                    key={p.id}
                    className="bg-blue-900/50 border border-blue-700 text-blue-200 px-4 py-1.5 rounded-full text-sm font-medium"
                  >
                    {p.name}
                  </span>
                ))}
              ...
              <div className="grid grid-cols-2 gap-4 h-full">
                {roundDetails.matches.map((match) => {
                  ...
                  return (
                    <div
                      key={match.id}
                      className="bg-[#0f2347] border border-blue-900 rounded-2xl p-5 flex flex-col justify-between"
                    >
                      {/* Court label */}
                      <div className="flex items-center justify-between mb-4">
                        ...
                        {hasResult && (
                          <span className="text-xs bg-green-900/50 text-green-400 border border-green-800 px-2 py-0.5 rounded-full font-semibold">
                            ✓ הסתיים
                          </span>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        {/* Team A */}
                        <div className={`flex-1 text-right rounded-xl p-3 transition-all ${aWon ? 'bg-blue-600/20 border border-blue-500' : 'bg-blue-900/20 border border-blue-900'}`}>
                          <p className={`font-bold text-base leading-tight ${aWon ? 'text-white' : 'text-blue-200'}`}>
                            {match.team_a_player_1.name}
                          </p>
                          <p className={`font-bold text-base leading-tight mt-1 ${aWon ? 'text-white' : 'text-blue-200'}`}>
                        ...
                        <div className="flex flex-col items-center gap-1 px-2 shrink-0">
                          {hasResult ? (
                            <>
                              <span className={`text-3xl font-black tabular-nums ${aWon ? 'text-white' : 'text-blue-600'}`}>
                                {match.score_a}
                              </span>
                              <span className="text-blue-800 text-xs">—</span>
                              <span className={`text-3xl font-black tabular-nums ${bWon ? 'text-white' : 'text-blue-600'}`}>
                                {match.score_b}
                              </span>
                            </>
                          ) : (
                        ...
                        {/* Team B */}
                        <div className={`flex-1 text-left rounded-xl p-3 transition-all ${bWon ? 'bg-blue-600/20 border border-blue-500' : 'bg-blue-900/20 border border-blue-900'}`}>
                          <p className={`font-bold text-base leading-tight ${bWon ? 'text-white' : 'text-blue-200'}`}>
                            {match.team_b_player_1.name}
                          </p>
                          <p className={`font-bold text-base leading-tight mt-1 ${bWon ? 'text-white' : 'text-blue-200'}`}>
